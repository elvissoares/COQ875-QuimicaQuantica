...
"""
STO-1s wavefunction for a hydrogen-like atom
"""
def STO1s(r,zeta=1.0):
    return np.sqrt(zeta**3/np.pi) * np.exp(-zeta * r)

"""
Gaussian-type orbital (GTO) for a given alpha
"""
def GTO(r,alpha):
    return np.power(2 * alpha / np.pi,3/4.0) * np.exp(-alpha * r**2)

"""
Pople set (STO-$n$G)
"""
def STO1G(r):
    return GTO(r,alpha=0.270950)

def STO2G(r):
    return 0.678914*GTO(r,alpha=0.151623) + 0.430129*GTO(r,alpha=0.851819)

def STO3G(r):
    return 0.444635*GTO(r,alpha=0.109818) + 0.535328*GTO(r,alpha=0.405771) + 0.154329*GTO(r,alpha=2.22766)
```
