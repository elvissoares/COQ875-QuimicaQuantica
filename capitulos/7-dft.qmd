...
Definindo um potencial efetivo de Kohn-Sham como
...
$$
\left[-\frac{1}{2}\nabla^2 + V_\text{KS}(\boldsymbol{r}) \right]\psi_i
=
\sum_{j=1}^N
\epsilon_{ij}\psi_j.
$$
...
$$
\boxed{
\left[-\frac{1}{2}\nabla^2 + V_\text{KS}(\boldsymbol{r}) \right]
\phi_i(\boldsymbol{r})
=
\epsilon_i
\phi_i(\boldsymbol{r})
...
4. Calcule o potencial efetivo de Kohn-Sham $V_\text{KS}(\boldsymbol{r})$;

5. Resolva as equações de Kohn-Sham para a nova densidade $\rho^{n+1}(\boldsymbol{r}) = \sum_i |\psi_i(\boldsymbol{r})|^2$;
